# Parse COasthma_4266.txt
# I figured out the ALLEELE1 is EA allele by looks at table s3 from the paper (https://www.sciencedirect.com/science/article/pii/S0002929719300679?via%3Dihub#!)
# Also the effect size column is labeled BETA but they are actually OR
...
#gwas_data <- fread("~/infer_mutational_bias/data/GWAS_ATLAS/COasthma_4266.txt")

# Filter to P-values smaller than 5e-8
data <- gwas_data %>% filter(P_BOLT_LMM_INF <= 5e-8)
...
# Re-name columns
colnames(data) <- c("SNP", "CHR", "BP", "EA","NEA", "EAF", "OR", "SE", "P")
