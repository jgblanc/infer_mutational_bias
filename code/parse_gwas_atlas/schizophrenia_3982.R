...
ggplot(data=data_plot, aes(x=RAF,y=abs(log(OR)))) + geom_point(alpha=0.5) + theme_bw() +
theme(plot.title = element_text(hjust = 0.5)) + ylab(TeX('$\\hat{\\beta}$')) +
xlab("Risk Alllele Frequency (RAF)") + theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 11), axis.title.y = element_text(size = 15))
