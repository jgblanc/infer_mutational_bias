...
5. I pulled out the SNPs with D/A info from the 1kG files and did LD pruning on these SNPs using plink --indep-pairwise 50 5 0.95  
...
MAF filter 0.01 (n=139818, pruned = 47249)
```{r}
loading <- fread("../data/STRAT/chr1_EUR_0.01.eigenvec.var.DA.txt")
loading <- loading %>% na.omit() # Get rid of no info (DA) SNPs 
pruned <- fread("../output/STRAT/pruned/chr1_EUR_0.01.eigenvec.var.DA.prune.in", header = F)
...
MAF filter 0.05 (n=123616, pruned = 40998)
```{r}
loading <- fread("../data/STRAT/chr1_EUR_0.05.eigenvec.var.DA.txt")
loading <- loading %>% na.omit() # Get rid of no info (DA) SNPs 
pruned <- fread("../output/STRAT/pruned/chr1_EUR_0.05.eigenvec.var.DA.prune.in", header = F)
loading <- left_join(pruned,loading, by = c("V1"="V2"))
only_num <- loading[,5:24]
sums <- colMeans(only_num, na.rm = T)
stan_devs <- apply(only_num, 2, sd)
...
MAF filter 0.1 (n = 110300, pruned = 36269)
```{r}
loading <- fread("../data/STRAT/chr1_EUR_0.1.eigenvec.var.DA.txt")
loading <- loading %>% na.omit() # Get rid of no info (DA) SNPs 
pruned <- fread("../output/STRAT/pruned/chr1_EUR_0.1.eigenvec.var.DA.prune.in", header = F)
loading <- left_join(pruned,loading, by = c("V1"="V2"))
only_num <- loading[,5:24]
sums <- colMeans(only_num, na.rm = T)
stan_devs <- apply(only_num, 2, sd)
...
MAF filter 0.2 (n=86234, pruned = 28011)
```{r}
loading <- fread("../data/STRAT/chr1_EUR_0.2.eigenvec.var.DA.txt")
loading <- loading %>% na.omit() # Get rid of no info (DA) SNPs 
pruned <- fread("../output/STRAT/pruned/chr1_EUR_0.2.eigenvec.var.DA.prune.in", header = F)
loading <- left_join(pruned,loading, by = c("V1"="V2"))
only_num <- loading[,5:24]
sums <- colMeans(only_num, na.rm = T)
stan_devs <- apply(only_num, 2, sd)
...
MAF filter 0.3 (n=602011, pruned = 19875)
```{r}
loading <- fread("../data/STRAT/chr1_EUR_0.3.eigenvec.var.DA.txt")
loading <- loading %>% na.omit() # Get rid of no info (DA) SNPs 
pruned <- fread("../output/STRAT/pruned/chr1_EUR_0.3.eigenvec.var.DA.prune.in", header = F)
loading <- left_join(pruned,loading, by = c("V1"="V2"))
only_num <- loading[,5:24]
sums <- colMeans(only_num, na.rm = T)
stan_devs <- apply(only_num, 2, sd)
...
MAF filter 0.4 (n=33130, pruned = 10576)
```{r}
loading <- fread("../data/STRAT/chr1_EUR_0.4.eigenvec.var.DA.txt")
loading <- loading %>% na.omit() # Get rid of no info (DA) SNPs 
pruned <- fread("../output/STRAT/pruned/chr1_EUR_0.4.eigenvec.var.DA.prune.in", header = F)
loading <- left_join(pruned,loading, by = c("V1"="V2"))
only_num <- loading[,5:24]
sums <- colMeans(only_num, na.rm = T)
stan_devs <- apply(only_num, 2, sd)
